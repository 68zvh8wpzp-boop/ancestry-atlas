...
(()=>{
  ...
  try{
    ...
    if(typeof targetYForCenter==='function'){
      targetYForCenter=()=>{
        const size=viewport();
        if(body.classList.contains('aa-phone'))return size.height*(size.height>size.width?0.72:0.61);
  ...
  function syncSurfaceState(){
    const open=!landing?.classList.contains('hidden')||chooser?.classList.contains('open')||story?.classList.contains('open')||film?.classList.contains('open');
  ...
  [landing,chooser,story,film].filter(Boolean).forEach(element=>surfaceObserver.observe(element,{attributes:true,attributeFilter:['class','aria-hidden']}));

  syncSurfaceState();
  syncMobileDock();
  apply({recenter:true});
  window.AncestryExperience={version:'3.8.0',refresh:apply,viewport};
})();
